...
This function runs the soupx tool for removing ambient RNA "soup" from your samples
...
outdir: Provide the path to a folder that already exists (please make the folder if it's not already made) where you would like to store record-keeping related outputs. This is not the path that will be used to store the seurat object

seuout: Provide the path to a folder that already exists (please make the folder if it's not already made) where you would like to store the post-processing seurat object
...
scdc_decon <- function(sc_ref, st_query, sc_meta, out){
  ...
  saveRDS(st_query, paste(path, 'SCDC_deconvolution_', sc_meta, '_', Sys.Date(), sep = ''))
  
  save(sc_ref, st_query, markers_sc, eset_SC, eset_ST, file = paste(path, 'Workspace_Image_SCDC_deconvolution_', sc_meta, '_', Sys.Date(), sep = ''))
}
```
